...
class FaqMatcherService
{
    ...
    public function match(
        ...
        /*
         * If lexical retrieval found nothing, keep the AI semantic
         * layer useful by providing a bounded FAQ pool from the same
         * agency (or the whole active knowledge base). The AI still
         * cannot invent an answer; it can only select an existing FAQ.
         */
        if ($faqs->isEmpty()) {
            $usedFallbackPool = true;
            $fallbackQuery = Faq::query()->with('agency');

            if ($agencyId !== null) {
                $fallbackQuery->where('agency_id', $agencyId);
            }
        ...
        $scored =
            $scored->filter(
                fn (Faq $faq) =>
                    $usedFallbackPool ||
                    $faq->match_score >= self::MIN_CANDIDATE_SCORE
            );
